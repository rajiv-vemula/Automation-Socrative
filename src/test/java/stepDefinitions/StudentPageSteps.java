...
public class StudentPageSteps {
	...
	{
		AssertJUnit.assertEquals(title, studentPage.getTitleWindowName());
	}
	...
	{
		AssertJUnit.assertTrue(studentPage.verifyRoomName());
		AssertJUnit.assertTrue(studentPage.verifyFirstName());
	}
	...
	{
		AssertJUnit.assertEquals(popup,studentPage.verifyPopup());
	}
	...
	{
		AssertJUnit.assertTrue(studentPage.verifyFeedbackContainer());
	}
	...
	{
		AssertJUnit.assertTrue(studentPage.verifyWaitingText());
	}
	
	@Then("Student closes the browser")
	public void student_closes_the_browser() throws InterruptedException
	{
		Thread.sleep(2000);
		driver.quit();
	}
